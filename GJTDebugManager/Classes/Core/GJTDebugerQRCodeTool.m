...
@interface GJTDebugerQRCodeTool () <AVCaptureMetadataOutputObjectsDelegate,CAAnimationDelegate>
...
@implementation GJTDebugerQRCodeTool
...
- (UIImageView *)scanLineView
{
    if (!_scanLineView) {
        ...
        _scanLineView.image = [UIImage debuger_imageNamed:@"debuger_scan_line"];
...
- (void)addAnimation{
    ...
    CABasicAnimation *animation = [GJTDebugerQRCodeTool moveYTime:self.QRCodeWidth/250 * 2.5 fromY:[NSNumber numberWithFloat:-10] toY:[NSNumber numberWithFloat:self.QRCodeWidth-30] rep:OPEN_MAX];
...
void DebugerProviderReleaseData (void *info, const void *data, size_t size){
    free((void*)data);
}
...
+ (UIImage*)imageBlackToTransparent:(UIImage*)image withRed:(CGFloat)red andGreen:(CGFloat)green andBlue:(CGFloat)blue{
    ...
    // 输出图片
    CGDataProviderRef dataProvider = CGDataProviderCreateWithData(NULL, rgbImageBuf, bytesPerRow * imageHeight, DebugerProviderReleaseData);
